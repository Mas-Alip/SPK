...
class DashboardFrame(tk.Frame):
    ...
    def build_ui(self):
        header = tb.Label(self, text="Dashboard", font=("Helvetica", 18, "bold"))
        header.pack(pady=12)

        # stats frame
        self.stats_frame = tb.Frame(self)
        self.stats_frame.pack(fill="x", padx=16, pady=8)

        self.lbl_total = tb.Label(self.stats_frame, text="Total Nasabah: -", font=("Helvetica", 12))
        self.lbl_total.grid(row=0, column=0, padx=8, pady=6, sticky="w")

        self.lbl_kriteria = tb.Label(self.stats_frame, text="Total Kriteria: -", font=("Helvetica", 12))
        ...
        tb.Button(quick, text="Kelola Nasabah", command=self.go_nasabah).pack(side="left", padx=6)
        tb.Button(quick, text="Kelola Kriteria", command=self.go_kriteria).pack(side="left", padx=6)
        tb.Button(quick, text="Perhitungan", command=self.go_perhitungan).pack(side="left", padx=6)
